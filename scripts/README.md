...
- [Production Deployment Links](#production-deployment-links)
- [Deployment Configuration](#deployment-configuration)
- [Docker Compose File Strategy](#docker-compose-file-strategy)
...
React + Vite frontend DigitalOcean App Platform üzerinde yayınlanır.

```text
https://finalcase-frontend-5psfu.ondigitalocean.app/
```

### Canlı Backend

Backend mikroservisleri AWS Elastic Beanstalk Docker platformunda çalıştırılır. Public backend entry point API Gateway’dir.

```text
http://finalcase-backend.eu-central-1.elasticbeanstalk.com
```

> Not: Mevcut Elastic Beanstalk domain’i HTTP çalışır. HTTPS frontend’den HTTP backend’e browser request atıldığında mixed content problemi çıkabilir. Tam production entegrasyonu için backend daha sonra HTTPS ile yayınlanmalıdır: custom domain + ACM certificate + load-balanced Elastic Beanstalk veya Cloudflare/reverse proxy çözümü.

### CI/CD

GitHub Actions build/test/deploy pipeline’ları:

```text
https://github.com/<your-github-owner>/n11-Bootcamp-Finalcase/actions
```
...
| Backend | AWS Elastic Beanstalk Docker Platform |
| Database | AWS RDS PostgreSQL |
| Container Registry | AWS Elastic Container Registry |
| Deploy Notification | Slack Incoming Webhook |
| Local Observability | Prometheus + Grafana + Loki + Jaeger |

### Local Observability Linkleri

Local Docker Compose observability stack ayağa kaldırıldığında:

| Tool | URL | Amaç |
|---|---|---|
| Grafana | `http://localhost:3000` | Metrics/log dashboard |
| Prometheus | `http://localhost:9090` | Metrics scrape/query |
| Jaeger | `http://localhost:16686` | Distributed tracing |
| RabbitMQ Management | `http://localhost:15672` | Queue/exchange debug |
| Eureka Dashboard | `http://localhost:8761` | Service registry |
| API Gateway | `http://localhost:8080` | Local backend entry point |
...
Repository’ye secret commit edilmez. Aşağıdaki değerler GitHub Repository Secrets içinde tutulur:
...
Secret olmayan CI/CD ayarları GitHub Repository Variables içinde tutulur:

```text
AWS_REGION=eu-central-1
EB_APPLICATION_NAME=finalcase-backend
EB_ENVIRONMENT_NAME=Finalcase-backend-env
EB_DEPLOY_BUCKET=finalcase-eb-deploy-onatsubasi-0965-eu-central-1
...
> `ECR_REPOSITORY_PREFIX` secret değildir. Repository variable olarak tanımlanmalıdır. Environment variable veya secret olarak eklenirse workflow tarafından okunamayabilir.

### AWS Elastic Beanstalk Environment Properties

Backend runtime secret/env değerleri Elastic Beanstalk Environment Properties içinde tutulur:
...
AWS Console
→ Elastic Beanstalk
→ Finalcase-backend-env
→ Configuration
→ Software
→ Environment properties
```

### DigitalOcean Frontend Environment Variables

Frontend public build-time değişkenleri DigitalOcean App Platform env alanında tutulur:
...
Frontend env içine `JWT_SECRET`, `POSTGRES_PASSWORD`, `IYZICO_SECRET_KEY`, `AWS_SECRET_ACCESS_KEY`, `SLACK_WEBHOOK_URL` gibi secret değerler konmaz.

---

## Docker Compose File Strategy

Bu projede compose dosyalarının sorumlulukları ayrıdır:

| Dosya | Amaç |
|---|---|
| `docker-compose.yml` | Local full stack: backend servisleri + PostgreSQL + RabbitMQ + Prometheus/Grafana/Loki/Jaeger |
| `docker-compose.lite.yml` | Local lightweight stack: düşük RAM için observability olmadan backend denemesi |
| `deploy/aws/elasticbeanstalk/docker-compose.aws-eb.template.yml` | GitHub Actions tarafından render edilip AWS Elastic Beanstalk’a deploy edilen production template |

`infra/` klasörü compose dosyası tutmaz. Sadece config/provisioning dosyaları için kullanılır:
...
Production deploy’da root `docker-compose.prod.yml` kullanılmaz. AWS deploy için kaynak dosya:
...
`./scripts` klasörü local geliştirme ve debug için yardımcı scriptler içerir:

| Script | Amaç |
|---|---|
| `build-local-images.ps1` | Windows/PowerShell ile local Jib image build |
| `debug-service.sh` | Bir container/service için log, health ve env debug |
| `wait-ready.sh` | Docker Compose servisleri hazır olana kadar bekleme |
| `watch-status.sh` | `docker compose ps` ve `docker stats` izleme |

Örnek kullanım:

```bash
./scripts/debug-service.sh checkout-service
./scripts/wait-ready.sh
./scripts/watch-status.sh
```

Windows local image build:
...
Canlı demo sırasında mağazanın boş görünmemesi için seed data kullanılabilir. Seed data; kategori, marka, ürün, inventory stock ve Search read-model projection kayıtları oluşturur.

Seed dosyaları:

```text
infra/postgres/seed/01_demo_catalog_inventory_search_seed.sql
infra/postgres/seed/02_demo_queries.sql
```

### Ne zaman çalıştırılır?

1. Backend servisleri en az bir kez çalışmış olmalı.
2. Flyway migration’lar ilgili tabloları oluşturmuş olmalı.
3. RDS `finalcasedb` database’ine pgAdmin ile bağlanılmalı.
4. Query Tool içinde `01_demo_catalog_inventory_search_seed.sql` çalıştırılmalı.
5. Sonra `02_demo_queries.sql` ile kayıtlar kontrol edilmeli.
...
### Demo arama beklentileri

```text
phone   → iPhone + Samsung
iphone  → iPhone
galaxy  → Samsung
running → Nike Running Shoes
...
Bu seed dosyası demo kolaylığı içindir. Direct SQL seed işlemi servis eventlerini bypass eder. Normal production davranışında ürünler Catalog Service admin endpointleriyle oluşturulmalı, Catalog eventleri RabbitMQ üzerinden Search projection’ı güncellemelidir.

Eğer seed SQL çalışırken `column does not exist`, `relation does not exist` veya enum/status hatası alınırsa, mevcut Flyway migration tablo kolonlarıyla seed dosyası küçük bir revizyon ister.
